...
{
	TELLERSERVICE_PARAMS_t *p = &teller_service_params[tellerID];   
	...
	if (pdPASS == xQueueReceive(queue, &nextCustomer, 0))
	{
		...
		nextCustomer.service_end_time = nextCustomer.service_start_time + random_time;
		
		// Write back to the list of all customers because queues copy data and are not directly linked to the original
		for (int i = *(p->CustomerPtrSize) - 1; i >= 0; i--)
		{
			// Find this customer
			if (p->CustomerPtr[i].arrival_time == nextCustomer.arrival_time)
			{
				p->CustomerPtr[i].service_start_time = nextCustomer.service_start_time;
				p->CustomerPtr[i].service_end_time = nextCustomer.service_end_time;
				break;
			}
		}
...
void StartBreak(int tellerID)
{
	...
	currBreak.end_time = currBreak.start_time + random_time;
